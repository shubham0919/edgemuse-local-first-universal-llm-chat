  ...
  useEffect(() => {
    ...
    const highlightCode = async () => {
      try {
        ...
        if ((shiki as any).createHighlighter) {
        ...
        } else if ((shiki as any).getHighlighter) {
          // Fallback for older shiki versions that still expose getHighlighter
          highlighter = await (shiki as any).getHighlighter({
            themes: ['github-light', 'github-dark'],
            langs: ['javascript', 'typescript', 'python', 'html', 'css', 'json', 'bash', 'markdown', 'tsx', 'jsx', 'go', 'rust'],
          });
        } else {
          throw new Error('shiki.createHighlighter/getHighlighter is not available');
        }
      } catch (error) {
      ...
      try {
        ...
        const html = (highlighter as any).codeToHtml(code, {
          lang: language,
          theme: theme,
        });
        if (isMounted) {
